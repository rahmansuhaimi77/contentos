...
type Asset = {
  id: string; brand_id: string; kind: AssetKind; title: string; storage_path: string;
  mime_type: string | null; notes: string | null; created_at: string; preview_url?: string;
};
type VisualProfile = {
  primary_color: string; secondary_color: string; accent_color: string; font_notes: string; visual_style: string; image_rules: string;
};

const assetKinds: Array<{ value: AssetKind; label: string }> = [
  { value: 'logo', label: 'Logo / brand mark' },
  { value: 'screenshot', label: 'App / website screenshot' },
  { value: 'vehicle', label: 'Vehicle / product photo' },
  { value: 'visual_reference', label: 'Visual style reference' },
  { value: 'other', label: 'Other' },
];
const emptyVisual: VisualProfile = { primary_color: '', secondary_color: '', accent_color: '', font_notes: '', visual_style: '', image_rules: '' };
...
export default function AssetsPage() {
  ...
  const selectedBrandName = brands.find((brand) => brand.id === brandId)?.name ?? 'Active brand';
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (selected) { setBrandId(selected.id); await loadBrandAssets(selected.id); }
      setLoading(false);
    }
  ...
  async function loadBrandAssets(id: string) {
    ...
    const [visualResult, assetResult] = await Promise.all([
      supabase.from('contentos_brand_visuals').select('primary_color,secondary_color,accent_color,font_notes,visual_style,image_rules').eq('brand_id', id).maybeSingle(),
      supabase.from('contentos_brand_assets').select('id,brand_id,kind,title,storage_path,mime_type,notes,created_at').eq('brand_id', id).order('created_at', { ascending: false }),
    ]);
  ...
  async function saveVisual(e: FormEvent) {
    ...
    const { error: saveError } = await supabase.from('contentos_brand_visuals').upsert({ brand_id: brandId, ...visual, updated_at: new Date().toISOString() }, { onConflict: 'brand_id' });
    setSavingVisual(false);
    if (saveError) { setError(saveError.message); return; }
    setMessage('Visual profile saved. New creative packs will use these rules.');
  }
  ...
  async function uploadAsset(e: FormEvent) {
    ...
    const { error: uploadError } = await supabase.storage.from('contentos-assets').upload(storagePath, file, { cacheControl: '3600', upsert: false, contentType: file.type });
    if (uploadError) { setUploading(false); setError(uploadError.message); return; }
    const { error: rowError } = await supabase.from('contentos_brand_assets').insert({
      brand_id: brandId, kind, title: title.trim(), storage_path: storagePath, mime_type: file.type, notes: notes.trim() || null,
    });
    if (rowError) {
    ...
    setUploading(false); setMessage('Approved brand asset uploaded.');
  }
  ...
  if (loading) return <section className="unifiedPage"><div className="dashboardSkeleton">Loading Brand Assets…</div></section>;
  ...
  return <section className="unifiedPage">
    <header className="pageHero compactHero"><div><span className="eyebrow">BRAND · ASSETS</span><h1>Give the creative engine the real brand.</h1><p>{selectedBrandName} · Approved visuals are the source of truth for creative production.</p></div><Link className="appPrimary" href="/brand">Back to Brand</Link></header>
    {message && <div className="notice">{message}</div>}
    {error && <div className="error globalError">{error}</div>}

    <div className="assetIntro panel"><div><span className="eyebrow">START HERE</span><h2>Upload real references before generating visuals.</h2><p>{isKampusRide
      ? 'Best first set: approved logo variants, KR app icon, real product screenshots, and visual references you actually like. ContentOS should use these references, not invent replacements.'
      : 'Best first set: approved logo, product screenshots/photos and visual references you actually like.'}</p></div></div>

    <section className="toolGrid assetToolGrid">
      <div className="assetForms">
        <form className="panel" onSubmit={saveVisual}>
          <div className="panelHead"><span>01</span><div><h3>Visual profile</h3><p>Define the visual system once.</p></div></div>
          <div className="colorFieldGrid">
            <label className="field"><span>Primary colour</span><input value={visual.primary_color} onChange={(e) => setVisual({ ...visual, primary_color: e.target.value })} placeholder="#123456" /></label>
          ...
          <label className="field"><span>Fonts / typography notes</span><textarea rows={3} value={visual.font_notes} onChange={(e) => setVisual({ ...visual, font_notes: e.target.value })} /></label>
          <label className="field"><span>Visual style</span><textarea rows={4} value={visual.visual_style} onChange={(e) => setVisual({ ...visual, visual_style: e.target.value })} /></label>
          <label className="field"><span>Image rules / never do</span><textarea rows={4} value={visual.image_rules} onChange={(e) => setVisual({ ...visual, image_rules: e.target.value })} /></label>
        ...
        <form className="panel" onSubmit={uploadAsset}>
          <div className="panelHead"><span>02</span><div><h3>Upload approved asset</h3><p>PNG, JPG, WEBP or SVG · maximum 5 MB.</p></div></div>
          ...
          <button className="generate" disabled={uploading || !file || !title.trim()}>{uploading ? 'Uploading…' : '+ Upload asset'}</button>
        </form>
      </div>

      <section className="panel">
        <div className="panelHead"><span>{assets.length}</span><div><h3>Approved reference library</h3><p>Visual source of truth for {selectedBrandName}.</p></div></div>
        {assets.length === 0 && <div className="emptyState">No brand assets yet. Upload the real logo first.</div>}
        <div className="assetGrid">{assets.map((asset) => <article className="assetCard" key={asset.id}>
          <div className="assetPreview">{asset.preview_url ? <img src={asset.preview_url} alt={asset.title} /> : <span>Preview unavailable</span>}</div>
          <div className="assetCardBody"><div className="knowledgeMeta"><span>{assetKinds.find((item) => item.value === asset.kind)?.label || asset.kind}</span><small>{new Date(asset.created_at).toLocaleDateString()}</small></div><h4>{asset.title}</h4>{asset.notes && <p>{asset.notes}</p>}<button className="assetDelete" onClick={() => removeAsset(asset)}>Delete</button></div>
        </article>)}</div>
      </section>
    </section>
  </section>;
