...
function installStoryboard(input: QuickCreateInput, context: ProductionVisualContext): StoryboardScene[] {
  ...
  const scenes: Array<Omit<StoryboardScene, 'image_prompt'>> = [
    { scene: 1, duration: 'Slide 1', visual: 'Simple cover slide with Android and iPhone visual cues and the approved KampusRide app icon.', on_screen_text: 'How to Install KampusRide 📱', voiceover: 'Tak perlu cari dekat App Store atau Play Store. Install terus dari browser.' },
    { scene: 2, duration: 'Slide 2', visual: 'Android phone mockup showing KampusRide opened in Google Chrome. Keep the browser controls recognisable and uncluttered.', on_screen_text: 'Android: Open KampusRide in Chrome', voiceover: 'Buka KampusRide guna Google Chrome.' },
    { scene: 3, duration: 'Slide 3', visual: 'Android Chrome menu tutorial showing the three-dot menu, Install app, then Install. Use callouts/arrows, not fabricated extra UI.', on_screen_text: '⋮ → Install app → Install', voiceover: 'Tap tiga titik, pilih Install app, kemudian Install.' },
    { scene: 4, duration: 'Slide 4', visual: 'iPhone Safari tutorial showing Share, Add to Home Screen, Open as Web App, then Add. Keep the sequence visually obvious.', on_screen_text: 'Safari → Share → Add to Home Screen', voiceover: 'Untuk iPhone, buka dalam Safari, tap Share, Add to Home Screen, hidupkan Open as Web App, kemudian Add.' },
    { scene: 5, duration: 'Slide 5', visual: 'KampusRide notification permission moment with a clear Allow callout. Use an approved real prompt/screenshot when available.', on_screen_text: 'Enable Notifications 🔔 → Allow', voiceover: 'Lepas install, buka KampusRide dan tap Allow bila notification permission keluar supaya tak terlepas driver offer, ride updates dan trip status.' },
    { scene: 6, duration: 'Slide 6', visual: 'Phone Home Screen showing the approved KR app icon installed successfully. Simple positive completion state.', on_screen_text: 'Done ✅  From Campus, For You', voiceover: 'Siap. KampusRide dah ready dekat Home Screen.' },
  ];
  return scenes.map((scene) => ({ ...scene, image_prompt: `${base} ${scene.duration}: ${scene.visual} Preserve on-screen wording: “${scene.on_screen_text}”.` }));
}
